...
export async function POST(req: NextRequest) {
  try {
    ...
    logCompanyData('CU', '対象スプレッドシートのデータ取得', {
    ...
    for (let colIndex = 0; colIndex < maxColumns; colIndex++) {
      ...
      logCompanyData('CU', `列${columnLetter}のデータ処理`, {
        columnHeader,
      ...
      if (!conversationData || conversationData.trim() === '会話データ') {
        logCompanyData('CU', `列${columnLetter}に有効な会話データが見つかりません`);
        continue;
      }
      ...
      try {
        ...
        logCompanyData('CU', `列${columnLetter}の企業抽出結果`, {
        ...
        logCompanyData('CU', `列${columnLetter}の企業抽出エラー`, { error: extractError });
