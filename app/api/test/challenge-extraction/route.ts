...
export async function POST(req: NextRequest) {
  try {
    ...
    // 課題抽出APIを呼び出し
    const response = await fetch(`${req.nextUrl.origin}/api/challenges/extract`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json({
      success: true,
      testData,
      challengeExtractionResult: result,
      summary: {
        status: response.status,
        success: result.success,
        extractedChallengesCount: result.extractedChallenges?.length || 0,
    ...
    console.error('課題抽出テストエラー:', error);
