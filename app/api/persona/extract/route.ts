...
// 議事録からペルソナを抽出するプロンプト
const PERSONA_EXTRACTION_PROMPT = `あなたは商談議事録から「業種×ペルソナ（商材に対するニーズ）」を抽出するアシスタントです。

【重要】ペルソナの定義：
・ペルソナ = 「クライアント様のサービスで解決できる課題」= 「商材に対するニーズ」
・「自社が抱えている課題」ではない
・「クライアント様のサービスを必要としている顧客の課題」を抽出する

抽出ルール：
・業種は「クライアントが狙いたい業界/業種」と明言したものだけ抽出。推定はしない。
・ペルソナは「クライアントのサービスで解決できる課題（商材に対するニーズ）」を、議事録の言い回しで保持。
...
2) ペルソナ抽出：「クライアント様のサービスで解決できる課題（商材に対するニーズ）」として語られている文を抽出し、原文を persona_statement_raw に入れる。
   【重要】「自社が抱えている課題」ではなく「クライアント様のサービスを必要としている顧客の課題」を抽出する。
...
・ペルソナ＝クライアント様のサービスで解決できる課題（商材に対するニーズ）のみ
・「自社が抱えている課題」ではなく「クライアント様のサービスを必要としている顧客の課題」を抽出する`
        }
      ],
      temperature: 0.1,
      max_tokens: 2000,
