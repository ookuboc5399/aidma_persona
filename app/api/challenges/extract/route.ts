...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`Processing filtered conversation data: ${processedConversationData.length} characters`);
    
    // GPT-4oの高いトークン制限を活用（128,000トークン ≈ 96,000文字）
    const estimatedTokens = processedConversationData.length * 0.75; // 概算トークン数
    console.log(`Estimated tokens after filtering: ${Math.round(estimatedTokens)}`);

    let chunks: string[];
    if (estimatedTokens <= 100000) {
      // 十分小さい場合は分割せずに一度に処理
      chunks = [processedConversationData];
      console.log(`Processing filtered conversation in single request (${processedConversationData.length} chars)`);
    } else {
      // 大きすぎる場合は分割処理
        ...
        try {
          ...
          const requestOptions: any = {
            ...
            messages: [
              {
                role: "system",
                content: `あなたは企業の事業課題を抽出・分析する専門家です。与えられた会話データから、その企業が抱えている具体的な事業課題をJSON形式で抽出してください。
  ...
  "challenges": [
    {
      ...
      "title": "課題のタイトル（簡潔に）",
      "description": "課題の詳細な説明（具体的な状況や影響を含む）",
      "urgency": "高/中/低",
      "keywords": ["関連キーワード1", "関連キーワード2"]
    }
  ],
  "summary": "${chunks.length === 1 ? '企業の事業課題全体のサマリー' : 'この会話断片から推測される事業課題のサマリー'}"
}

${chunks.length > 1 ? '注意：これは会話データの一部分です。断片的な情報から推測できる事業課題のみを抽出してください。' : '会話データ全体を分析して、包括的な事業課題分析を行ってください。'}`
              },
              {
                role: "user",
